...
`Integration1stHalfWithWallRiemann`执行如下操作（src\shared\particle_dynamics\fluid_dynamics\fluid_integration.hpp）：

```cpp
// 1. initialization() - 更新半步密度、压力、位置
rho_[index_i] += drho_dt_[index_i] * dt * 0.5;
p_[index_i] = fluid_.getPressure(rho_[index_i]);
pos_[index_i] += vel_[index_i] * dt * 0.5;

// 2. interaction() - 搜索邻居表，计算压力梯度力
...
`Integration2ndHalfWithWallRiemann`执行如下操作：

```cpp
// 1. initialization() - 更新半步位置
pos_[index_i] += vel_[index_i] * dt * 0.5;

// 2. interaction() - 计算速度散度引起的密度变化
