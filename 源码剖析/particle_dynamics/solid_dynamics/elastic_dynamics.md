# elastic_dynamics：类清单、关系与适用范围（含关键公式）
...
该模块实现“弱可压（weakly compressible）弹性固体”的显式 SPH 时间积分框架，核心是：
...
$$
\Delta t_i = \mathrm{CFL}\cdot \min\left(
...
$$
\mathbf v_i^{n+1} = \mathbf v_i^{n} + \Delta t\,\frac{\mathbf f_i + \mathbf f_i^{prior}}{m_i}.
$$
...
- `StressPK1OnParticle`：记作 $\mathbf P_i\mathbf B_i^{T}$（源码变量 `stress_PK1_B_`，具体在各 initialization 中填充）。
...
$$
\dot\varepsilon_{ij} \approx \left(\frac{D}{r_{ij}}\right)^2 (\mathbf x_i-\mathbf x_j)\cdot(\mathbf v_i-\mathbf v_j),
$$
...
$$
\mathbf f_i \sim \sum_{j\in\mathcal N(i)} \frac{m_i}{\rho_0}
...
**适用**：这是最“标准”的第一半步形式。后面的 PK2/Cauchy/Kirchhoff 只是改变“如何在半步构型上构造 $\mathbf P$”。
...
$$
exttt{stress\_PK1\_B}_i \leftarrow \mathbf P_i^{n+1/2}\,(\cdot)\,\mathbf B_i^{(\cdot)}.
$$
...
$$
	exttt{stress\_PK1\_B}_i = \mathbf P_i\,\mathbf B_i^T.
$$

#### (b) `Integration1stHalfCauchy`

用 Cauchy 应力 $\boldsymbol\sigma$（由 Almansi 应变给出）转换到 PK1：
...
$$
\mathbf f^{shear}_{ij} \propto \mu\,(J_i^{-2/d}+J_j^{-2/d})\,\frac{\mathbf x_i-\mathbf x_j}{r_{ij}}.
$$
...
2) 通过速度梯度计算 $\dot{\mathbf F}$（源码 SPH 梯度 + 修正矩阵）：

$$
\dot{\mathbf F}_i \approx \left[-\sum_{j\in\mathcal N(i)} (\mathbf v_i-\mathbf v_j)\otimes(\nabla W_{ij}V_j)^T\right]\,\mathbf B_i.
$$

3) 再把 $\mathbf F$ 推进到整步（源码 `update()` 是半步增量）：

$$
\mathbf F_i^{n+1} = \mathbf F_i^{n+1/2} + \tfrac12\Delta t\,\dot{\mathbf F}_i^{(\cdot)}.
$$

（$\dot{\mathbf F}$ 的时间层取法依赖外层调度；从代码结构看是“先 interaction 写 dF_dt，再 update 加半步”。）
