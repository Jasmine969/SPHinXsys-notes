...
{
    ParticleType *particles = base_particles_keeper_.createPtr<ParticleType>(*this); // 1
    ...
    particles->initializeBasicDiscreteVariables(); // 4
    sph_adaptation_->initializeAdaptationVariables(*particles); // 5
    // MatterMaterial与各MaterialProperty分别初始化局部参数。 // 6
    return *particles;
};
```
...
4. 调用`particles->initializeBasicDiscreteVariables()`。进入`initializeBasicDiscreteVariables()`：
   ...
   退出`initializeBasicDiscreteVariables()`。

5. 调用`sph_adaptation_->initializeAdaptationVariables(*particles)`。如果没有特别adaptation，什么也不做。

6. 主体`MatterMaterial`与附加的`MaterialProperty`分别初始化其局部参数。旧版只通过单个`base_material_`完成这一阶段的说明已经过时。
