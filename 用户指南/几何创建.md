...
- `void addABox(Transform transform, const Vecd &halfsize, ShapeBooleanOps op);`。添加一个矩形，用户需指定矩形的 `transform`、半尺寸和布尔运算类型。
    ...
    multi_polygon_.addABox(T, halfsize, ShapeBooleanOps::add);
    ```
...
`AlignedBox`类继承于`GeometricBox`类，区别在`AlignedBox`多了一个对齐轴（alignment axis），所有“上/下边界、周期映射、近边界判断”等操作都沿着这个对齐轴来做。
