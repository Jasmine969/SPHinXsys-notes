...
file(GLOB CONFIGURE_DEPENDS CASE_SOURCES
...
# --- Runtime input folder + regression tool copy (done at build time)
# 定义运行时 input 目录：bin/input。
set(CASE_INPUT_DIR "${CASE_BIN_DIR}/input")
# 给该 target 增加一个“构建后命令”（POST_BUILD）：每次该 target 链接完成后执行。
add_custom_command(TARGET ${TARGET_NAME} POST_BUILD
    COMMAND "${CMAKE_COMMAND}" -E make_directory "${CASE_INPUT_DIR}" # 用 cmake -E创建bin/input目录（跨平台，避免写mkdir）
    COMMAND "${CMAKE_COMMAND}" -E copy_directory
            "${CMAKE_CURRENT_SOURCE_DIR}/regression_test_tool" 
            "${CASE_INPUT_DIR}" # 把源码目录下的regression_test_tool/整个目录拷贝到bin/input/
    VERBATIM # 让 CMake 正确处理参数转义/空格，避免路径中有空格时命令被拆坏，更稳。
)
```
