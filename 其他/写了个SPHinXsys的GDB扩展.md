...
![](https://fengimages-1310812903.cos.ap-shanghai.myqcloud.com/20251222210244.png)

这样看着很费事，至少要点开三级节点才能看到变量名字。现在，加入了我的扩展之后，可以非常清晰地看到每一个vector的数据类型：

![](https://fengimages-1310812903.cos.ap-shanghai.myqcloud.com/20251222214150.png)

再点开每一个节点，就能看到变量名了：

![](https://fengimages-1310812903.cos.ap-shanghai.myqcloud.com/20251222214219.png)

是不是非常方便呢？

# 如何加入SPHinXsys扩展

需要两步。第一步，在`~/.gdbinit`中加入以下内容：

```
python
import sys
sys.path.insert(0, '</path/to/your/sphinxsys/source/dir>')
from tools.gdb.printers import register_SPHinXsys_printers
register_SPHinXsys_printers(None)
end
```

第二步，在SPHinXsys源码目录下添加一个`tools/gdb`的目录，里面新建一个`printers.py`文件，填入以下内容（代码400多行，太长了，放最后）。
...
and get a grouped list of variable names.
